...
class WindowManager {
	...
	constructor(callerId, hideKrunkerWindowsOnShow = true){
		...
		document.addEventListener("DOMContentLoaded", () => {
			if (!document.getElementById("nika-windowHolder")){
				let w = document.createElement("div");
				w.setAttribute("id", "nika-windowHolder");
				w.setAttribute("style", "display: none;");
				w.innerHTML = '<div id="nika-menuWindow"></div>';
				...
				document.getElementsByTagName("body")[0].addEventListener("click", e => {
					// @ts-ignore
					(!e.path.find(p => p.id === "nika-menuWindow" || p.id === this.callerId)) && this.hide();
				});
			}
		});
	}
	 ...
	 */
	setContent(content){
		document.getElementById("nika-menuWindow").innerHTML = content;
	}
	...
	show(){
		...
		document.getElementById("nika-windowHolder").setAttribute("style", "display: block;");
		this.shown = true;
	}
	 ...
	 */
	hide(){
		document.getElementById("nika-windowHolder").setAttribute("style", "display: none;");
		this.shown = false;
	}
